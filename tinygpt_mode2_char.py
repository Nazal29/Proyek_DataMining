"""
=============================================================
TinyGPT - Mode 2: Character-Level Tokenization
=============================================================
...
# Konfigurasi
# ─────────────────────────────────────────────
TOKENIZER_MODE = "char"      # ← Mode tokenisasi karakter
CORPUS_FILE    = "corpus.txt"

block_size    = 64           # lebih panjang karena char-level
embedding_dim = 64
n_heads       = 4
n_layers      = 3
lr            = 1e-3
...
# ─────────────────────────────────────────────
# Bangun vocabulary dari karakter unik
chars     = sorted(set(text))
vocab_size = len(chars)

# Buat mapping karakter <-> integer
...
print(f"\n[Tokenizer] Mode          : Character-Level")
print(f"[Tokenizer] Vocabulary    : {vocab_size} karakter unik")
print(f"[Tokenizer] Total token   : {len(ids):,}")
print(f"[Tokenizer] Rasio         : {char_count / len(ids):.3f} char/token")

# Contoh tokenisasi
sample = "dynamic pricing"
sample_tokens = [stoi.get(c, -1) for c in sample]
print(f"\n[Contoh Char] '{sample}' → {sample_tokens}")
print(f"[Contoh Char] Karakter unik: {chars[:20]}... (total {vocab_size})")
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("  HASIL GENERASI TEKS - Mode Character")
print("=" * 60)
...
for prompt in prompts:
    ...
    out = model.generate(context, max_new_tokens=120, temperature=0.8)
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("  ANALISIS PERFORMA - Mode Character")
print("=" * 60)
print(f"  Mode tokenisasi  : Character-Level")
